...
class QuizGenerator:
    ...
    def generate_question_with_vectorstore(self):
        ...
        if not self.llm:
            self.init_llm()  # Initialize the LLM if not already done
        
        
        # Raise an error if the vectorstore is not initialized on the class
        if not self.vectorstore:
            raise ValueError("Vectorstore is not initialized on the class. Please provide a vectorstore instance.")
        ...
        chain = {"context": retriever, "topic": RunnablePassthrough()} | prompt_template | self.llm
