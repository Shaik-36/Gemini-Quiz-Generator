...
class QuizGenerator:
    def __init__(self, topic=None, num_questions=1, vectorstore=None):
        ...
        # Initializes the QuizGenerator with a required topic, the number of questions for the quiz topic and context
        if not topic:
    ...
    def init_llm(self):
        ...
        # Initializing and configures the Large Language Model (LLM) for generating quiz questions.
 
        self.llm = VertexAI(
            model_name = "gemini-pro",   # LLM model used for the training
            temperature = 0.8,           # parameter that influences the language model's output, determining whether the output is more random and creative or more predictable. 
            max_output_tokens = 500      # Max num of limit for the total number of tokens
        )
    ...
    def generate_question_with_vectorstore(self):
        ...
        :returns: A JSON object representing the generated quiz question.
        """
        if not self.llm:
            self.init_llm()
    ...
    def validate_question(self, question: dict) -> bool:
        ...
        # Check if 'question' key exists (assuming question text is stored there)
        if 'question' not in question:
            return False

        # Loop through existing questions and compare text
        for existing_question in self.question_bank:
            if existing_question['question'] == question['question']:
              return False

        # No duplicates found, question is unique
        return True
